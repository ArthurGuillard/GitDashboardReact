...
import {USER_REDUCER_ACTION_SET_LAST_SEARCH} from "../reducer/searchDataReducer";
import {SEARCH_HISTORY_REDUCER_ACTION_PUSH, SearchHistoryContext} from "../context/SearchHistoryContext";
...
const buildLastSearchPayload = (gitHubResponse) => {
...
export const GHRResearcher = () => {
  ...
  const addToHistory = (newData) => dispatchSearchHistory({type:SEARCH_HISTORY_REDUCER_ACTION_PUSH, payload: newData});

  const required = (value) => (value ? undefined : "Username is missing");
  const maxSize = (value) => (value.length < 30 ? undefined : "Username is too long (> 30 characters)");
  const composeValidators = (...validators) => value => validators.reduce((error, validator) => error || validator(value), undefined);

  const handleSubmit = async ({usernameField}) => {
  ...
  const disableResearchButton = (submitting, metaError) => {
